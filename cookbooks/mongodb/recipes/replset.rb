...
mongo_arbiter = @node[:db_host] # "na"

if @node[:mongo_replset]
  mongo_nodes = @node[:mongo_utility_instances]

  # Chef::Log.info "Setting up Replica set: #{node[:mongo_replset]} \n mongo_nodes: #{mongo_nodes.inspect} \n executing on node #{@node[:name]}"

  if (@node[:name].match(/_0$/) && (mongo_nodes.length == 3 || (mongo_nodes.length == 2 && mongo_arbiter.length > 3)))
